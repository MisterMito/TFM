...
2. Variance filtering (`HighVarGeneSelector` → `var_quantile`).
3. `log1p` transformation.
4. Post-log feature selection (`VarianceThresholdFilter`).
5. Scaling (`StandardScaler`).
6. Optional PCA (`pca_var_threshold=0.9`).

## Code patterns
- See `genomics_dl/features_sklearn.py` for the feature transformer implementations.
- See `genomics_dl/models/train_binary.py` and `genomics_dl/models/train_multiclass.py`
  for the full training pipelines.
